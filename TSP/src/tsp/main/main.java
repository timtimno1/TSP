...
public class main
{
	...
	public static void main(String[] args)
	{
		...
		but.start();
		/*
		 * for(int i=0;i<d1.A.length;i++) { System.out.println(d1.s.get(d1.A[i])); }
		 */
		for (int k = 0; k < d1.A.length; k++)
		{
			...
			for (Integer i : temp)
			{
				...
				for (int j = 0; j < d1.A[k].size(); j++)
				{
					...
					if (min > W[i - 1][(int) d1.A[k].get(j) - 1]
							+ D[(int) d1.A[k].get(j) - 1][temp1.isEmpty() ? (int) (Math.pow(2, vnearNumber - 1)) - 1
									: (int) d1.s.get(temp1)])
					{
						min = W[i - 1][(int) d1.A[k].get(j) - 1]
								+ D[(int) d1.A[k].get(j) - 1][temp1.isEmpty() ? (int) (Math.pow(2, vnearNumber - 1)) - 1
										: (int) d1.s.get(temp1)];
		...
		for (int i = 0; i < d1.A[d1.A.length - 1].size(); i++)
		{
			...
			if (min > W[0][(int) d1.A[d1.A.length - 1].get(i) - 1]
					+ D[(int) d1.A[d1.A.length - 1].get(i) - 1][temp1.isEmpty()
							? (int) (Math.pow(2, vnearNumber - 1)) - 1
							: (int) d1.s.get(temp1)])
			{
				min = W[0][(int) d1.A[d1.A.length - 1].get(i) - 1]
						+ D[(int) d1.A[d1.A.length - 1].get(i) - 1][temp1.isEmpty()
								? (int) (Math.pow(2, vnearNumber - 1)) - 1
								: (int) d1.s.get(temp1)];
		...
		// disp.print(d1.A);
		/*
		 * v.add("V2"); v.add("V3"); int i=0; for(String str:v) { s.put(str, i++); }
		 * for(String str:v) { data[(int) (s.get(str))]=5; } for(int j:data) {
		 * System.out.print(j+ " "); }
		 */
	}
}
...
class Horse extends Thread
{
	public void run()
	{
		JOptionPane.showMessageDialog(null,
				"預計最久需要計算" + (Math.pow(main.vnearNumber, 2) * Math.pow(2, main.vnearNumber) * 0.0000000235) + "秒", "提示",
